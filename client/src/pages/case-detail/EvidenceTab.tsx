          ...
          {caseData.documents.map((doc) => {
            ...
            return (
              <div key={doc.id} className="flex flex-wrap items-center gap-x-4 gap-y-2 p-4">
                ...
                <button onClick={() => setPreviewDoc(doc)} className="p-1.5 text-slate-400 hover:text-blue-600 transition-colors" title="Preview" aria-label={`Preview ${doc.originalName}`}>
                  <Eye className="w-4 h-4" />
                </button>
                <button onClick={() => deleteMutation.mutate(doc.id)} disabled={deleteMutation.isPending} className="p-1.5 text-slate-400 hover:text-red-500 transition-colors" title="Delete" aria-label={`Delete ${doc.originalName}`}>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose} role="dialog" aria-modal="true" aria-label={`Preview: ${doc.originalName}`}>
      <div className="bg-white rounded-2xl shadow-xl max-w-4xl w-full max-h-[90vh] flex flex-col m-4" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div className="text-sm font-semibold text-slate-800 truncate">{doc.originalName}</div>
          <button ref={closeRef} onClick={onClose} className="p-1 text-slate-400 hover:text-slate-600" aria-label="Close preview">
            <X className="w-5 h-5" />
          </button>
        </div>
        <div className="flex-1 overflow-auto p-6">
          {error ? (
            ...
            <div className="text-sm text-slate-500 text-center py-12">Preview isn't available for this file type — use Download to open it.</div>
          ) : !blobUrl ? (
            ...
            <iframe src={blobUrl} className="w-full h-[70vh] rounded-lg border border-slate-200" title={doc.originalName} />
          )}
        </div>
