    ...
    <LookupCard<CourtHistoryResult>
      ...
      render={(result) => result.error ? (
        <div className="text-xs text-muted-foreground">
          <p>{result.error}</p>
          {result.scraperNote && <p className="mt-1 text-muted-foreground italic">{result.scraperNote}</p>}
        </div>
      ) : (
        <>
          ...
          {result.found && result.cases.length > 0 && (
            <div className="space-y-1 max-h-40 overflow-y-auto">
              {result.cases.slice(0, 6).map((c, i) => (
                <div key={i} className="flex gap-2 text-xs p-1.5 bg-muted rounded border border-border">
                  <span className="text-muted-foreground shrink-0 font-mono">{c.caseIndex}</span>
                  <span className="text-muted-foreground truncate">{c.plaintiff} v. {c.defendant}</span>
                  <span className="text-muted-foreground shrink-0">{c.status}</span>
                </div>
              ))}
              {result.cases.length > 6 && (
                <p className="text-xs text-muted-foreground">+{result.cases.length - 6} more — verify at iapps.courts.state.ny.us</p>
              )}
            </div>
          )}
          <p className="text-xs text-muted-foreground">Verify at: <strong>iapps.courts.state.ny.us/webcivil/FCASMain</strong></p>
        </>
      )}
    />
  );
