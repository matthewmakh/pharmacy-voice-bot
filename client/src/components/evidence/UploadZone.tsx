      ...
      >
        ...
        {uploading ? (
          <div className="flex flex-col items-center gap-3">
            ...
            <div className="text-sm font-medium text-foreground">Uploading and analyzing documents...</div>
            <div className="text-xs text-muted-foreground">This may take a moment. Each file is being classified by AI.</div>
          </div>
        ) : isDragActive ? (
          <div className="flex flex-col items-center gap-3">
        ...
        <div className="text-xs text-muted-foreground flex items-center gap-1.5">
          <div className="flex items-center gap-1">
            {acceptedFiles.slice(0, 3).map((f) => (
              <span key={f.name} className="flex items-center gap-1 bg-muted px-2 py-1 rounded">
                <FileIcon mimeType={f.type} />
                {f.name.length > 20 ? f.name.slice(0, 20) + '...' : f.name}
              </span>
            ))}
            {acceptedFiles.length > 3 && (
              <span className="text-muted-foreground">+{acceptedFiles.length - 3} more</span>
            )}
          </div>
        </div>
      )}
